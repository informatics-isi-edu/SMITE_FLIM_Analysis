...
prompt = {'Please enter the name of your data set:','Please enter the intensity threshold value:','Please enter the number of z stacks (1 for 2D images):', 'Please enter the number of channels used:'};
answer = inputdlg(prompt);
dataFolder = string(answer(1));
NADH_thresh_val =str2double(answer(2));
zStackAmount = str2double(answer(3));
channelAmount = str2double(answer(4));
ref_folder = fullfile(pwd,dataFolder);
cond_folder = dir(ref_folder);
figures = [];
figuresg = [];

save_fig_folder = fullfile(pwd, dataFolder+ "_Output");
if ~exist(save_fig_folder,'dir')
    mkdir(save_fig_folder)
end

%dataFolder = "Dropbox (EITM)\Shared-Dropbox\Mathias_training\Leica-Tiff-Data\20191112_13002ORG-H2BGFP_lowEGF_CRCmedia_Tiff-test";  % dataFolder: location of the folder that contains Data.
...
Excel_file = 'Analysis_test.xlsx';   % Name of the excel file to store all the data. Do not omit the ".xlsx"
...
mask_thresh = 0;            % Set to your batch masking value if batch_threshold_mask is select. Otherwise set as 0.
%%  Filtering proceure.
...
% Additional settings:
               % Threshold value for NADH in phasor analysis.
...
phasor_mode_calculating  =  0; % To calculate phasor based on mode, assign varable as 1, if not asign as 0

phasor_cen_calculating  =  1;  % To caluclaed phasor based on centroid, assign varable as 1, if not asign as 0
...
image_folder = "SavedFigures_test";     % Name of the figures folder.  All images will be stored in the figure folder. 
...
for i = 4: numel(imageFolder)  % Looping through the different individual folders
...
%     
    ...
    for z_cur = 1: z_stack
        ...
        int = imread(fullfile(image_file(1).folder,image_file((z_cur-1)*channelAmount*total_detector+(NADH_dec-1)*channelAmount+1).name));
        G = standardPhase( imread(fullfile(image_file(1).folder,image_file((z_cur-1)*channelAmount*total_detector+(NADH_dec-1)*channelAmount+3).name)));
        S = standardPhase( imread(fullfile(image_file(1).folder,image_file((z_cur-1)*channelAmount*total_detector+(NADH_dec-1)*channelAmount+4).name)));
...
%         end
        ...
        elseif phasor_cen_calculating == 1
            [G_cur, S_cur] = findCenPhasor(org_struct); % Finding the center of the phasor.
        else
