...
%          
%          % test
%          cur_int = double(imread(fullfile(char_add(1:slash_ind(end)-1),files{j})));
%          cur_G = standardPhase(double(imread(fullfile(char_add(1:slash_ind(end)-1),files{j+1}))));
%          cur_S = standardPhase(double(imread(fullfile(char_add(1:slash_ind(end)-1),files{j+2}))));
...
%        filefolder = fullfile((head_folder{i}),,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '.xlsx');   % Name of the excel file. 
       filefolder = fullfile(output_folder,char_short_add(1:slash_ind_short_add(end)-1),string(files{j}(1:end-4)) + '_ch1.csv');   % Name of the excel file. 
       writetable(DataTable,filefolder)
...
%        cond = [cond;string(char_list_of_fold(add_ind(end-3)+1:add_ind(end-2)-1))];
       ...
       % for ISS
       filefolder = fullfile((head_folder{i}),[char_list_of_fold(add_ind(end-1)+1:add_ind(end)-1),'_average_values_ch1.csv']);
       writetable(DataTable,filefolder)
...
%          cum_int=[cum_int (cur_int(:))'];
       ...
       % Applying median filter for the phasor plot: 
       z_struct = medfiltPhasor(z_struct,3);
%        z_struct = wavefiltPhasor(z_struct,2); 
       end   
...
%        clear cond name G_av_ch1 S_av_ch1
       ...
       % Applying median filter for the phasor plot: 
       current_struct = medfiltPhasor(current_struct,3);
%        current_struct = wavefiltPhasor(current_struct,2); 
       end
       current_struct = threshPhasorStruct(current_struct, thresh_val); 
       plotPhasorFast(current_struct);
       [Organoid_ave_G,Organoid_ave_S] = findCenPhasor(current_struct);  % finding the mean of G and S. 
...
%        filefolder = fullfile((head_folder{i}),,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '.xlsx');   % Name of the excel file. 
       filefolder = fullfile(output_folder,char_short_add(1:slash_ind_short_add(end)-1),string(files{j}(1:end-4)) + '_ch2.csv');   % Name of the excel file. 
       writetable(DataTable,filefolder)
...
%      cond = [cond;string(char_list_of_fold(add_ind(end-3)+1:add_ind(end-2)-1))];
       ...
       % for ISS
       filefolder = fullfile((head_folder{i}),[char_list_of_fold(add_ind(end-1)+1:add_ind(end)-1),'_average_values_ch2.csv']);
       writetable(DataTable,filefolder)
...
%        cond_find=strfind(char_add(1:slash_ind(end)-1),'_');
       ...
       DataTable=table(replicate_group,treatment,Sample_ID,name,G_av_ch1, S_av_ch1);
%        filefolder = fullfile((head_folder{i}),,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '.xlsx');   % Name of the excel file. 
       filefolder = fullfile(figures_folder,string(files{j}(1:end-4)) + '_ch1.csv');   % Name of the excel file. 
       writetable(DataTable,filefolder)
...
%      cond = [cond;string(char_list_of_fold(add_ind(end-3)+1:add_ind(end-2)-1))];
       ...
       % for ISS
       filefolder = fullfile((head_folder{i}),[char_list_of_fold(add_ind(end)+1:end),'_average_values_ch1.csv']);
       writetable(DataTable,filefolder)
...
%        filefolder = fullfile((head_folder{i}),,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '.xlsx');   % Name of the excel file. 
       filefolder = fullfile(figures_folder,string(files{j}(1:end-4)) + '_ch1.csv');
       writetable(DataTable,filefolder)
...
%        cond_find=strfind(char_add(1:slash_ind(end)-1),'_');   
       ...
       DataTable=table(replicate_group,treatment,Sample_ID,name,G_av_ch2, S_av_ch2);
%        filefolder = fullfile((head_folder{i}),,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '.xlsx');   % Name of the excel file. 
       filefolder = fullfile(figures_folder,string(files{j}(1:end-4)) + '_ch2.csv');
       writetable(DataTable,filefolder)
...
%     multiplane_file_name=char(fullfile(figures_folder,['mp','.tif']));
    ...
    if (ome_tif_activate=='y')
     ...
     bfsave(multiplaneData,multiplane_ome_name);
       ...
       DataTable=table(replicate_group,treatment,Sample_ID,name,G_av_ch1, S_av_ch1);
%        filefolder = fullfile((head_folder{i}),,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '.xlsx');   % Name of the excel file. 
       filefolder = fullfile(figures_folder,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '_ch1.csv');   % Name of the excel file. 
       writetable(DataTable,filefolder)
       G_av_ch1=[];
       S_av_ch1=[];
       cond = [];
...
%        clear cond name G_av_ch1 S_av_ch1
       ...
       DataTable=table(replicate_group,treatment,Sample_ID,name,Organoid_ave_G, Organoid_ave_S);
       % for ISS
       filefolder = fullfile((head_folder{i}),[char(files{((j-1)*3)+1}(1:und_ind(end)-1)),'_',(char_list_of_fold(add_ind(end)+1:end)),'_ch1.csv']);
       writetable(DataTable,filefolder)
...
%     multiplane_file_name=char(fullfile(figures_folder,['mp','.tif']));
    ...
    saveastiff((multiplaneData),multiplane_file_name,options);
     ...
     bfsave(multiplaneData,multiplane_ome_name);
       ...
       DataTable=table(replicate_group,treatment,Sample_ID,name,G_av_ch1, S_av_ch1);       
%        filefolder = fullfile((head_folder{i}),,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '.xlsx');   % Name of the excel file. 
       filefolder = fullfile(figures_folder,string(files{((j-1)*3)+1}(1:und_ind(end)-1)) + '_ch2.csv');   % Name of the excel file. 
       writetable(DataTable,filefolder)
       G_av_ch1=[];
       S_av_ch1=[];
       cond = [];
...
%        clear cond name G_av_ch1 S_av_ch1
       ...
       % for ISS
       filefolder = fullfile((head_folder{i}),[char(files{((j-1)*3)+1}(1:und_ind(end)-1)),'_',(char_list_of_fold(add_ind(end)+1:end)),'_ch2.csv']);
       writetable(DataTable,filefolder)
...
for kk=1:numel(list_of_fold)
  ...
  if ((kk==1 ||  ~strcmp(head_folder{kk},head_folder{kk-1})) && numel(dir(fullfile(head_folder{kk},"*.csv")))~=0)
     
  excell_ref_files = dir(fullfile(head_folder{kk},"*.csv"));  
  ...
  cum_excell_name_ch1=[commonPart(1:underline_ind(end)-1),'_organoid_stats_ch1.csv'];
  cum_excell_name_ch2=[commonPart(1:underline_ind(end)-1),'_organoid_stats_ch2.csv'];
commonPart='';
clear strings
for h=1:numel(excell_ref_files)
 ...
 if (sum(size(mergedTable_ch1))~=0)
       ...
       writetable(mergedTable_ch1,filefolder)    
       mergedTable_ch1 = table();
 end
  ...
  if (sum(size(mergedTable_ch2))~=0)
       ...
       writetable(mergedTable_ch2,filefolder)    
       mergedTable_ch2 = table();
 end
...
for ff=1:numel(list_of_fold)
    ...
    excell_cond_files = dir(fullfile(cond_fold{ff},subFolderNames{nn},"*.csv"));  
if (~isempty(excell_cond_files))
  ...
  if (sum(size(mergedTable_ch1))~=0)
       ...
       filefolder = fullfile(cond_fold{ff},'all_replicate_stats_ch1.csv');
       writetable(mergedTable_ch1,filefolder)    
       mergedTable_ch1 = table();
  end
  if (sum(size(mergedTable_ch2))~=0)
       ...
       filefolder = fullfile(cond_fold{ff},'all_replicates_stats_ch2.csv');
       writetable(mergedTable_ch2,filefolder)    
       mergedTable_ch2 = table();
  end  
...
for q=1:numel(replicate_group)
   ...
   if (q==1 ||  ~strcmp(replicate_group{q},replicate_group{q-1}))  
    ...
    excell_rep_group_files = dir(fullfile(replicate_group{q},rep_subFolderNames{p},"*.csv")); 
   ...
   for gg=1:numel(excell_rep_group_files)
    ...
    clear data
       ...
       filefolder = fullfile(replicate_group{q},'all_replicate_group_stats_ch1.csv');
       writetable(mergedTable_ch1,filefolder)    
       mergedTable_ch1 = table();
       end
       ...
       filefolder = fullfile(replicate_group{q},'all_replicate_group_stats_ch2.csv');
       writetable(mergedTable_ch2,filefolder)    
       mergedTable_ch2 = table();
       end      
